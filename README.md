基于深度Q网络DDQN的智能网络路由系统，用于在动态网络环境中进行数据包路由决策。

## 项目简介

本项目实现了一个使用深度强化学习进行网络路由的仿真系统。系统通过DQN算法学习最优路由策略，能够在动态变化的网络环境中高效地路由数据包。项目支持多种网络拓扑（包括Barabasi-Albert网络和卫星网络），并提供了完整的训练、测试和可视化功能。
